...
export default function CarouselSection({ recommendations }: Props) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSlide(([prev]) => [(prev + 1) % recommendations.length, 1])
    }, 5000)
              ...
              <div className="flex flex-col max-w-xl gap-2">
                <div className="hidden gap-2 md:flex">
                  ...
                  {slide?.taxonomy?.Genre.map(({ name, slug }) => (
                    <span
                      key={slug}
                      className="bg-zinc-700 text-white text-[10px] md:text-xs px-3 py-1 rounded-full"
                    >
                      {name}
                    </span>
                  ))}
                ...
                <div className="flex gap-6 text-sm text-zinc-300">
                  <span className="flex gap-1">
                    <Star className="w-4 h-4 fill-yellow-400 text-white" />{' '}
                    {slide?.user_rate}
                  </span>
                  <span className="flex gap-1">
                    <Book className="w-4 h-4 text-white" />{' '}
                    {slide?.latest_chapter_number} Chapters
                  </span>
                  <span className="hidden md:flex gap-1">
                    <Crown className="w-4 h-4 fill-yellow-400" /> Rank #
                    {slide?.rank}
                  </span>
                </div>
        ...
        {recommendations.map((_, i) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-300 ${
              i === index ? 'w-6 bg-blue-500' : 'w-2 bg-zinc-500'
            }`}
          />
        ))}
      </div>
