...
export default function HistoryPage() {
  ...
  const filteredHistory = history.filter((h) =>
    h.comic_title.toLowerCase().includes(searchQuery.toLowerCase()),
  )
        ...
        <div className="w-full md:w-1/2 lg:w-1/4 flex items-center justify-between gap-2">
          <div className="flex items-center justify-start w-full h-10 bg-gray-400/20 rounded-md px-2">
            <Input
              ...
              className="w-full h-full aria-selected:ring-0 focus-visible:ring-0 border-0 pl-1 text-[12px] md:text-sm"
            />
          </div>
        </div>
      </section>
