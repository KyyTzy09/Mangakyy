import SortHistoryButton, {
  type SortType,
} from '@/features/history/components/buttons/SortHistoryButton'
...
import {
  getHistoryByComicId,
  type NewChapterHistory,
  type NewComicHistory,
...
export default function HistoryDetailPage() {
  const params = useParams({ from: '/history-detail/$mangaId' })
  const router = useRouter()

  const history = getHistoryByComicId(params.mangaId)
  const [sortedChapters, setSortedChapters] = useState<NewChapterHistory[]>(
    history?.chapters || [],
  )
    ...
    <div className="flex flex-col items-start justify-start min-h-screen bg-linear-to-br from-[#0f172a] via-[#0b1a33] to-black/80 text-slate-100 pt-20 md:pt-24 p-2 px-5 gap-2 md:gap-5 pb-16 md:pb-0">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-4 items-start justify-start w-full gap-4">
        {sortedChapters?.map((chapter, i) => (
          <ChapterHistoryCard
            key={i}
            history={history as NewComicHistory}
            chapter={chapter}
            index={i}
          />
        ))}
